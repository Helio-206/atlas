  ...
  return (
    <section className="mt-5 border-t border-[var(--border)] pt-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {canUpload ? (
          <form action={uploadDocumentAction} className="flex items-center gap-2" encType="multipart/form-data">
      ...
      <div className="mt-3 divide-y divide-[var(--border)] border-y border-[var(--border)]">
        {documents.length === 0 ? (
          <p className="py-4 text-[12px] text-[var(--text-muted)]">Nenhum documento anexado.</p>
        ) : documents.map((document) => (
          <a className="grid min-h-11 grid-cols-[1fr_auto_auto] items-center gap-4 py-2 text-[12px] hover:bg-[var(--surface-subtle)]" href={`/api/documents/${document.id}/download`} key={document.id}>
