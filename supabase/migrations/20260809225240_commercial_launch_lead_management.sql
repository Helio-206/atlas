alter table public.demo_requests
  drop constraint if exists demo_requests_status_check;

alter table public.demo_requests
  add column if not exists internal_notes text,
  add column if not exists updated_at timestamptz not null default now();

alter table public.demo_requests
  add constraint demo_requests_status_check
  check (status in ('new','contacted','qualified','demo_scheduled','pilot_proposed','won','lost'));

alter table public.demo_requests
  add constraint demo_requests_internal_notes_check
  check (internal_notes is null or char_length(internal_notes) <= 4000);

create index if not exists demo_requests_status_created_at_idx
  on public.demo_requests(status, created_at desc);

create or replace function platform.require_demo_request_admin()
returns void
language plpgsql
stable
security definer
set search_path = ''
as $$
begin
  if auth.uid() is null then
    raise exception using errcode = '42501', message = 'authentication_required';
  end if;

  if not exists (
    select 1
    from identity.memberships membership
    where membership.user_id = auth.uid()
      and membership.role = 'administrator'
      and membership.status = 'active'
  ) then
    raise exception using errcode = '42501', message = 'administrator_required';
  end if;
end;
$$;

create or replace function platform.list_demo_requests(p_limit integer default 100)
returns table(
  id uuid,
  name text,
  company text,
  role text,
  email text,
  phone text,
  message text,
  status text,
  internal_notes text,
  created_at timestamptz,
  updated_at timestamptz
)
language plpgsql
stable
security definer
set search_path = ''
as $$
begin
  perform platform.require_demo_request_admin();
  if p_limit is null or p_limit < 1 or p_limit > 200 then
    raise exception using errcode = '22023', message = 'demo_request_limit_invalid';
  end if;

  return query
  select request.id, request.name, request.company, request.role, request.email,
    request.phone, request.message, request.status, request.internal_notes,
    request.created_at, request.updated_at
  from public.demo_requests request
  order by request.created_at desc, request.id desc
  limit p_limit;
end;
$$;

create or replace function platform.update_demo_request(
  p_id uuid,
  p_expected_updated_at timestamptz,
  p_status text,
  p_internal_notes text default null
)
returns timestamptz
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_updated_at timestamptz;
  v_notes text := nullif(btrim(coalesce(p_internal_notes, '')), '');
begin
  perform platform.require_demo_request_admin();
  if p_status not in ('new','contacted','qualified','demo_scheduled','pilot_proposed','won','lost') then
    raise exception using errcode = '22023', message = 'demo_request_status_invalid';
  end if;
  if v_notes is not null and char_length(v_notes) > 4000 then
    raise exception using errcode = '22023', message = 'demo_request_notes_invalid';
  end if;

  update public.demo_requests
  set status = p_status,
      internal_notes = v_notes,
      updated_at = now()
  where id = p_id
    and updated_at = p_expected_updated_at
  returning updated_at into v_updated_at;

  if v_updated_at is null then
    if not exists (select 1 from public.demo_requests request where request.id = p_id) then
      raise exception using errcode = 'P0001', message = 'demo_request_not_found';
    end if;
    raise exception using errcode = 'P0001', message = 'demo_request_conflict';
  end if;

  return v_updated_at;
end;
$$;

revoke all on function platform.require_demo_request_admin() from public, anon, authenticated;
revoke all on function platform.list_demo_requests(integer) from public, anon;
revoke all on function platform.update_demo_request(uuid, timestamptz, text, text) from public, anon;
grant execute on function platform.list_demo_requests(integer), platform.update_demo_request(uuid, timestamptz, text, text) to authenticated;

create or replace function public.list_demo_requests(p_limit integer default 100)
returns table(
  id uuid,
  name text,
  company text,
  role text,
  email text,
  phone text,
  message text,
  status text,
  internal_notes text,
  created_at timestamptz,
  updated_at timestamptz
)
language sql
stable
security invoker
set search_path = ''
as $$ select * from platform.list_demo_requests($1); $$;

create or replace function public.update_demo_request(
  p_id uuid,
  p_expected_updated_at timestamptz,
  p_status text,
  p_internal_notes text default null
)
returns timestamptz
language sql
security invoker
set search_path = ''
as $$ select platform.update_demo_request($1, $2, $3, $4); $$;

revoke all on function public.list_demo_requests(integer), public.update_demo_request(uuid, timestamptz, text, text) from public, anon;
grant execute on function public.list_demo_requests(integer), public.update_demo_request(uuid, timestamptz, text, text) to authenticated;
