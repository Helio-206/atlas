...
const accounts = {
  requester: 'requester@atlas.demo',
  technical: 'technical@atlas.demo',
  finance: 'finance@atlas.demo',
  director: 'director@atlas.demo',
  procurement: 'procurement@atlas.demo',
  warehouse: 'warehouse@atlas.demo',
...
async function switchUser(page, email) {
  await logout(page)
...
try {
  ...
  assert.ok(href)
  const signed = await context.request.get(`${baseUrl}${href}`, { maxRedirects: 0 })
