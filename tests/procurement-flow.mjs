...
try {
  ...
  await page.waitForURL((url) => url.hash === '#items')
  assert.equal(new URL(page.url()).hash, '#items')
  assert.equal(await page.locator('#items').isVisible(), true)
  ...
  await page.getByLabel('Comentário da decisão').fill('Technical E2E OK')
  await page.getByRole('button', { name: 'Aprovar' }).click()
  ...
  assert.match(await page.locator('main').innerText(), new RegExp(low.requestNumber))
  await page.goto(`${baseUrl}${low.detailPath}`)
  await page.getByLabel('Comentário da decisão').fill('Financial E2E OK')
  await page.getByRole('button', { name: 'Aprovar' }).click()
  ...
  await page.getByLabel('Comentário da decisão').fill('Detalhar especificação')
  ...
  await page.goto(`${baseUrl}${high.detailPath}`)
  await page.getByLabel('Comentário da decisão').fill('Executive E2E OK')
  await page.getByRole('button', { name: 'Aprovar' }).click()
